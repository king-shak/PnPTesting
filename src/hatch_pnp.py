...
def getTranslation(rotation):
    omega = np.radians(sd.getNumber('tx', 0))
    theta = np.radians(sd.getNumber('ty', 0))
    distance = DELTA_HEIGHT / np.sin(np.radians(theta + MOUNTING_ANGLE))
    x = distance * np.sin(rotation + omega)
...
def draw(img, corners, imgpts):
    ...
    img = cv2.line(img, corner, tuple(imgpts[2].ravel()), (0,255,255), 3)
...
while (True):
    ...
    if (ret == True) and (tv == 1):
        ...
        if (len(tcornx) == NUM_OF_POINTS) and (len(tcorny) == NUM_OF_POINTS):
            ...
            translation = getTranslation(angleToTarget)
            print(np.degrees(angleToTarget), translation)
